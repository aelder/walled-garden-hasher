...
/* N-lane wave kernel: nlanes independent hashes advance in lockstep, one
 * step per wave, executed in case-sorted order so the indirect dispatch
 * sees runs of identical targets. 2 <= nlanes <= VERUS_WAVE_MAX_LANES. */
#define VERUS_WAVE_MAX_LANES 64
void verusclhashv2_2_wave(
